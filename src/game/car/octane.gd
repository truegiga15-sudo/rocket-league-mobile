## WS46 -- Car Mesh Octane (budget-aware, deterministic)
## Authored Octane mesh loader: deterministic GLB from assets/authored/car/,
...
## Depends on: src/core/constants.gd (WS04), assets/authored/car/octane_mesh_a_v01.glb
extends Node3D
class_name Octane
const PC = preload("res://src/core/constants.gd")
const OCTANE_MESH_PATH: String = "res://assets/authored/car/octane_mesh_a_v01.glb"
const MESH_PATH: String = OCTANE_MESH_PATH
const AUTHORED_MESH_NAME: String = "octane_mesh_a_v01.glb"
...
var _octane_scene: PackedScene = null
...
func _ready() -> void:
	...
	if OS.is_debug_build():
		...
		if not v["ok"]:
			for e in v["errors"]:
				push_warning("[Octane] debug_validate: %s" % e)
func _load_mesh() -> void:
	if ResourceLoader.exists(OCTANE_MESH_PATH):
		var res: Resource = load(OCTANE_MESH_PATH)
		if res is PackedScene:
			_octane_scene = res as PackedScene
			var inst: Node = _octane_scene.instantiate()
			inst.name = "OctaneMesh"
			add_child(inst)
			_loaded = true
...
static func debug_validate() -> Dictionary:
	...
	if OCTANE_MESH_PATH != "res://assets/authored/car/octane_mesh_a_v01.glb":
		errors.append("MESH_PATH unexpected: %s" % OCTANE_MESH_PATH)
