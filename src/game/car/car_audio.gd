## WS54 — Car Engine Audio Attachment (budget-aware, deterministic)
## Engine audio attachment driven by WS14 engine + WS46 Octane mesh.
## No procedural generation — all audio authored/committed under assets/authored/car/.
...
##             src/core/input_service.gd (WS06), src/core/time_service.gd (WS05),
##             src/game/car/engine.gd (WS14), src/game/car/octane.gd (WS46)
...
# ---------------------------------------------------------------------------
# Authored audio constants — single source for WS54
# ---------------------------------------------------------------------------

## Authored engine loop asset (deterministic, committed).
const ENGINE_AUDIO_PATH: String = "res://assets/authored/car/audio_engine_loop_a_v01.ogg"
const AUDIO_PATH: String = ENGINE_AUDIO_PATH
const AUTHORED_AUDIO_NAME: String = "audio_engine_loop_a_v01.ogg"

## Audio bus — all car SFX through SFX (conventions §8: Master -> [Music, SFX, Crowd, UI]).
...
## Physics tick — must be 120 Hz (validated against PC + EngineRef + OctaneRef).
...
func update(throttle: float, speed_abs: float, delta: float, is_reverse: bool = false, is_boosting: bool = false) -> Dictionary:
	...
	var target_pitch := CarAudio.pitch_for_speed(t, speed_abs, is_reverse, is_boosting)
	var is_attacking := target_pitch > _pitch
	_pitch = CarAudio.smooth_pitch(_pitch, target_pitch, delta, is_attacking)
	_volume_db = CarAudio.volume_db_for_throttle(t)
	var ratio := clamp(speed_abs / EngineRef.MAX_SPEED_FORWARD, 0.0, 1.15) if EngineRef.MAX_SPEED_FORWARD > 0 else 0.0
	_rpm = CarAudio.rpm_for_throttle_speed(t, ratio)
...
# ---------------------------------------------------------------------------
# Attachment helper — attach engine audio to Octane car node (Node3D)
# ---------------------------------------------------------------------------

static func create_player(car_node: Node = null) -> AudioStreamPlayer3D:
	var player := AudioStreamPlayer3D.new()
	player.name = "CarEngineAudio"
...
static func attach_to_car(car_node: Node3D) -> AudioStreamPlayer3D:
	...
	var existing := car_node.get_node_or_null("CarEngineAudio") as AudioStreamPlayer3D
	if existing != null:
		return existing
...
static func get_octane_mesh_path() -> String:
...
static func debug_validate() -> Dictionary:
	...
	if not is_equal_approx(OctaneRef.CAR_LENGTH, 4.2):
		errors.append("Octane CAR_LENGTH %.2f != 4.2" % OctaneRef.CAR_LENGTH)
	if not is_equal_approx(OctaneRef.CAR_WIDTH, 2.1):
		errors.append("Octane CAR_WIDTH %.2f != 2.1" % OctaneRef.CAR_WIDTH)
	...
	if not ENGINE_AUDIO_PATH.begins_with("res://assets/authored/car/"):
		errors.append("ENGINE_AUDIO_PATH must be under assets/authored/car/")
...
static func perf_mark() -> Dictionary:
	return {"scope": "CarAudio", "budget_calls": BUDGET_CALLS, "max_calls": MAX_CALLS_PER_TICK, "tick_hz": TICK_HZ, "bus": AUDIO_BUS}

func perf_mark_instance() -> Dictionary:
	return {"scope": "CarAudio", "calls_last_tick": _call_count_last_tick, "budget": MAX_CALLS_PER_TICK, "pitch": _pitch, "volume_db": _volume_db, "budget_ok": _call_count_last_tick <= MAX_CALLS_PER_TICK}
